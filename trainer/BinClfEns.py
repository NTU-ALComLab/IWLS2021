...
from .svUtils import svTemplateTxt, svAssign, svModule, svVarGen, svBitPad
...
def bvPrep(nClass, clfList, nSumBit):
    predList = [c + '_pred' for c in clfList]
    sumList = ['sum_{}'.format(str(i)) for i in range(nClass)]

    vvars = svVarGen([('wire', 1, predList[i], 1) for i in range(len(clfList))]) \
